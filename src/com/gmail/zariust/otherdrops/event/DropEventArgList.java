...
public class DropEventArgList extends AbstractSequentialList<String> {
	public class Iterator implements ListIterator<String> {
		...
		public void add(String val) {
			...
			synchronized(DropEventArgList.this) {
				if(cur == null) {
	...
	public DropEventArgList() {
		//System.out.println(">> new DropEventArgList()");
		head = tail = null;
		size = 0;
	}
	
	public DropEventArgList(Collection<String> other) {
		//System.out.println(">> new DropEventArgList(" + other + ")");
		addAll(other);
	}
	
	public DropEventArgList(String... array) {
	...
	public static void main(String[] args) {
		...
		DropEventArgList list = new DropEventArgList();
		list.add("PING");
		list.add("PONG");
		list.add("STRING");
		list.add("CLANG");
		...
		DropEventArgList.Iterator iter1, iter2;
		...
		System.out.println(list.toString() + " " + list.size());
		for(String str : list) {
			if(str.startsWith("P")) list.remove(str);
		}
		System.out.println(list.toString() + " " + list.size());
...
//		System.out.println(list.toString() + " " + list.size());
		...
		System.out.println("---");
		list = new DropEventArgList("RADIUS=3","HEIGHT=5","QUACK","MOO");
		System.out.println(list.toString() + " " + list.size());
		for(String arg : list) {
	...
	private static void profile(Random rng, int n) {
		...
		for(int i = 0; i < n; i++) {
			...
			char[] arg = new char[5];
			rng.nextBytes(rand);
			for(int j = 0; j < 5; j++) arg[j] = (char)rand[j];
		...
		assert list.size() == n;
		DropEventArgList.Iterator iter = list.listIterator();
