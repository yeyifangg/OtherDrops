...
public class EndermanData extends CreatureData {
	...
	public void setOn(Entity mob, Player owner) {
		if (mob instanceof Enderman) {
			...
			if (this.md != null) ((Enderman)mob).setCarriedMaterial(md );
	...
	public boolean matches(Data d) {
		if(!(d instanceof EndermanData)) return false;
		EndermanData vd = (EndermanData)d;

		if (this.md != null)
			if (this.md != vd.md) return false;
		
		if (this.canCarry  != null)
			if (this.canCarry != vd.canCarry) return false;
		...
		if (!state.isEmpty() && !state.equals("0")) {
			...
			for (String sub : split) {
				...
				if (sub.equalsIgnoreCase("carry")) canCarry = true;
					...
					if (material != null && data == null) materialData = material.getNewData((byte) 0);
					else if (material != null) materialData = material.getNewData((byte) data.getData());
