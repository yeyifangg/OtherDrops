...
public class OcelotData extends CreatureData {
	...
	public void setOn(Entity entity, Player owner) {
		if (entity instanceof Ocelot) {
			...
			if (type != null) ocelot.setCatType(type);
			if (adult != null) if (!adult) ocelot.setBaby();
			if (tamed != null) {
				if (tamed)  ocelot.setOwner(owner);
			} else {
	...
	public boolean matches(Data d) {
		if(!(d instanceof OcelotData)) return false;
		...
		if (this.tamed != null)
			if (this.tamed != vd.tamed) return false; 
	...
	public static CreatureData parseFromString(String state) {
		...
		if (!state.isEmpty() && !state.equals("0")) {
			...
			for (String sub : split) {
				...
				else if (sub.equalsIgnoreCase("wild"))  tamed = false;
				else {
					// aliases
