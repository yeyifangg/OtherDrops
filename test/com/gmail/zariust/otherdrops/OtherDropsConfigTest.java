...
public class OtherDropsConfigTest {
	...
	private File getTempFile(String string) throws IOException {
	    ...
	    pw.println(string); pw.close(); fw.close();
	...
	void testDefaultsForFile(File file) {
		...
		if (yaml.getConfigurationSection("defaults") == null) {
			...
			System.out.println("list: "+yaml.getConfigurationSection("defaults").getKeys(true).toString());
			ConfigurationSection section = yaml.getConfigurationSection("defaults");
			for (String key : yaml.getConfigurationSection("defaults").getKeys(true)) {
	...
	// get a fake server - only real part is that .getLogger gets an actual logger object
	...
	Server getServer() {
		return new Server() {
			...
			public World getWorld(String arg0) {
				...
				if (arg0.equalsIgnoreCase("TestWorld")) return CustomDropTest.testWorld;
				else if (arg0.equalsIgnoreCase("SecondWorld")) return CustomDropTest.secondWorld;
				return null;
			}
